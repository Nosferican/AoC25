...
function step_down(a, b)
	a = input[1]
	S = findfirst(contains("(S|\|"), a)
	if isa(S, Integer)
		output = S
	else
		beam = findfirst()
...
states = Vector{typeof(input)}()
function go_down_row!(states, state)
	particules = findall(isequal("|"), state)
	row = maximum(elem.I[1] for elem in particules)
	idx = first(particule for particule in particules if particule.I[1] == row)
	col = idx.I[2]
	while row < (size(state, 1) - 1)
		particules = findall(isequal("|"), state)
		row = maximum(elem.I[1] for elem in particules)
		idx = first(particule for particule in particules if particule.I[1] == row)
		col = idx.I[2]
...
go_down_row!(states, input)
foreach(println, states)
states[1]
states[2]
states[3]
states[1]
...
input = input[1:4,:]
struct State
	output :: Union{Nothing,Vector{State}}
	function State(obj)
		output = State[]
		particules = findall(isequal("|"), input)
		row = maximum(elem.I[1] for elem in particules)
		idx = first(particule for particule in particules if particule.I[1] == row)
		col = idx.I[2]
		for row in row:(size(input, 1) - 1)
			if input[row + 1, col] == "."
				input[row + 1, col] = "|"
		...
		new(isempty(output) ? nothing : output)
	end
end
output = State(input)
```
