...
function calibrate_machine(machine)
    ...
    @satvariable(a[1:length(buttons)], Int)
    constrs = []
    # non-negativity (and optional upper bounds)
    for idx in eachindex(a)
        push!(constrs, a[idx] >= 0)
    end
    for i in eachindex(joltage)
        idxs = [j for j in eachindex(buttons) if buttons[j][i]]
        push!(constrs, sum(a[idxs]) == joltage[i])
    end
    n = maximum(joltage)
    status = true
    while status
        expr = and(constrs..., sum(a) == n)
        status, res = sat!(interactive_solver, expr)
        status = status ≠ :SAT
        n += 1
    end
...
input = split(input, '\n')
input = Dict(
    match(r"^\p{l}{3}", ln).match => collect(m.match for m in eachmatch(r"(\p{l}{3})", replace(ln, r"^\p{l}{3}\:\s" => "")))
    for ln in input
)
k = sort!(unique!(append!(collect(keys(input)), reduce(union, values(input)))))
mapper = Dict(k[idx] => idx for idx in eachindex(k))
# mapper_ = Dict(v => k for (k, v) in mapper)
g = DiGraph(length(k))
for (k, v) in input
    for elem in v
        add_edge!(g, mapper[k], mapper[elem])
    end
end
cache_out = Dict(findfirst(isequal("out"), k) => 1)
function n_paths!(cache, src, desto)
    if src == desto
        1
    else
        if haskey(cache, src)
            ...
            xxx = outneighbors(g, src)
            if !isempty(xxx)
                cache[src] = sum(n_paths!(cache, s, desto) for s in xxx)
            end
            0
        end
    end
end
n_paths!(cache_out, findfirst(isequal("you"), k), findfirst(isequal("out"), k))

output = 0
finaldest = Int[]
visited = Int[]
tovisit = outneighbors(g, findfirst(isequal("you"), k))
while !isempty(tovisit)
    for n in tovisit
        dest = outneighbors(g, n)
        if findfirst(isequal("out"), k) in dest
            output += 1
        end
        append!(tovisit, dest)
        push!(visited, n)
    end
    setdiff!(tovisit, visited)
end
```

```{julia}
input = readlines(joinpath(DIR_PRJ, "data", "day11_input.txt"))
input = Dict(
    match(r"^\p{l}{3}", ln).match => collect(m.match for m in eachmatch(r"(\p{l}{3})", replace(ln, r"^\p{l}{3}\:\s" => "")))
    for ln in input
)
...
svr = findfirst(isequal("svr"), k)
dac = findfirst(isequal("dac"), k)
fft = findfirst(isequal("fft"), k)
out = findfirst(isequal("out"), k)
cache_out = Dict(out => 1)
cache_dac = Dict(dac => 1)
cache_fft = Dict(fft => 1)
svr_dac = n_paths!(cache_dac, svr, dac)

sum(elem for elem in Int[])
...
for p in all_simple_paths(g, dsp_svr, dsp_out)
    println(p)
    output += 1
    output > 3 && break
end
...
out_connections = outneighbors.(Ref(g), eachindex(k))
in_connections = inneighbors.(Ref(g), eachindex(k))
in_connections_n = length.(in_connections)
in_connections_n[dsp_out]



dsp = dijkstra_shortest_paths(g, findall(∈(["svr", "dac", "fft"]), k));
dsp.dists




count(>(0), dsp_svr.dists)
count(>(0), dsp_dac.dists)
count(>(0), dsp_fft.dists)
findfirst(isequal("svr"), k)
svr = findfirst(isequal("svr"), k)
out = findfirst(isequal("out"), k)
dac = findfirst(isequal("dac"), k)
fft = findfirst(isequal("fft"), k)
# svr to fft
svr2dac = yen_k_shortest_paths(g, svr, dac, weights(g), 2_000);
svr2fft = yen_k_shortest_paths(g, svr, fft, weights(g), length(k));
dac2fft = yen_k_shortest_paths(g, dac, fft, weights(g), length(k));
fft2dac = yen_k_shortest_paths(g, fft, dac, weights(g), length(k));
dac2out = yen_k_shortest_paths(g, dac, out, weights(g), length(k));
fft2out = yen_k_shortest_paths(g, fft, out, weights(g), length(k));

maximum(length, svr2dac.paths)
filter(x -> dac ∉ x, svr2fft.paths)
...
struct SvrOutPath
    ...
    fft :: Bool
    visited :: Int[]
    function SvrOutPath(cur, dac, fft)
    end
end
...
@test output == 599
output = 0
struct PathFromSrvToOut
    curr :: Int
    dac :: Bool
    fft :: Bool
    function PathFromSrvToOut(curr, dac, fft)
        if curr == findfirst(isequal("out"), k)
            global output += dac & fft
            return nothing
        else
            dest = outneighbors(g, curr)
            dac = dac ? dac : (curr == findfirst(isequal("dac"), k))
            fft = fft ? fft : (curr == findfirst(isequal("fft"), k))
            for d in dest
                PathFromSrvToOut(d, dac, fft)
            end
        end
    end
end
PathFromSrvToOut(findfirst(isequal("svr"), k), false, false)
...
function all_paths(g::DiGraph, src::Int, dst::Int)
    """Find all paths from src to dst in directed graph g"""
    paths = Vector{Vector{Int}}()
    
    function dfs(current::Int, target::Int, path::Vector{Int})
        if current == target
...
all_paths_you_out = all_paths(g, findfirst(isequal("svr"), k), findfirst(isequal("out"), k))
all_paths_you_out = all_paths(g, findfirst(isequal("you"), k), findfirst(isequal("out"), k))


filter(path -> all(x -> x ∈ path, [findfirst(isequal("dac"), k), findfirst(isequal("fft"), k)]), all_paths_you_out)

findfirst(isequal("dac"), k) .∈ all_paths_you_out
