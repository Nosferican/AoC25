...
visited = Int[]
output = 0
next_node = unique(outneighbors(g, findfirst(isequal("you"), k)))
edges(g, findfirst(isequal("you"), k))
while !isempty(next_node)
    for next_node_adv in next_node
        if next_node_adv == findfirst(isequal("out"), k)
            output += 1
        else
            append!(next_node, unique(outneighbors(g, next_node_adv)))
        end
        push!(visited, next_node_adv)
    end
    next_node = setdiff(next_node, visited)
end



dijkstra_shortest_paths(g, srcs, distmx=weights(g));
count(>(0), dijkstra_shortest_paths(g, findfirst(isequal("you"), k)).pathcounts)
```