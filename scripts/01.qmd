...
@testset "Day 11 Part 1" begin
    ...
    you = findfirst(isequal("you"), k)
    out = findfirst(isequal("out"), k)
    am_g = adjacency_matrix(g)
    P = sum(am_g^n for n in 1:(size(g, 1) - 1))
    @test P[you, out] == 599
end
```
...
@testset "Day 11 Part 2" begin
    ...
    P = sum(am_g^n for n in 1:(size(g, 1) - 1))
    @test max(
        P[svr, dac] * P[dac, fft] * P[fft, out],
        P[svr, fft] * P[fft, dac] * P[dac, out]
    ) == 393474305030400
end
```
...
@testset "Day 12" begin
    input = readlines(joinpath(DIR_PRJ, "data", "day12_input.txt"))
    shapes = Matrix{Bool}[]
    regions = Vector{Int}[]
    idx = firstindex(input)
    while idx < length(input)
        if contains(input[idx], r"^(\d):$")
            push!(
                shapes,
                reduce(
                    vcat,
                    permutedims(
                        graphemes(input[x]) .== "#") for x in (idx+1):(idx+3)
                )
            )
            idx += 4
        elseif contains(input[idx], r"\d+")
            push!(regions, [parse(Int, m.match) for m in eachmatch(r"\d+", input[idx])])
            idx += 1
        else
            idx += 1
        end
    end
    @test count(9sum(region[3:end]) ≤ reduce(*, region[1:2]) for region in regions) == 546
end
```